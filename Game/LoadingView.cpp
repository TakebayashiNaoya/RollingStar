...
void LoadingView::Update()
{
	if (showLoadingPhases[enLoadingPhase_6]) {
		m_pushA_FontRender.SetText(L"PUSH (A) ");
		m_pushA_FontRender.SetPosition({ 450.0f,-350.0f,0.0f });
		m_pushA_FontRender.SetScale(2.0f);
		m_pushA_FontRender.SetColor({ 0.9f,0.9f,1.0f,1.0f });
...
void LoadingView::Render(RenderContext& rc)
{
	...
	if (showLoadingPhases[enLoadingPhase_1]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_1].Draw(rc);
	}
	if (showLoadingPhases[enLoadingPhase_2]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_2].Draw(rc);
	}
	if (showLoadingPhases[enLoadingPhase_3]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_3].Draw(rc);
	}
	if (showLoadingPhases[enLoadingPhase_4]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_4].Draw(rc);
	}
	if (showLoadingPhases[enLoadingPhase_5]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_5].Draw(rc);
	}
	if (showLoadingPhases[enLoadingPhase_6]) {
		m_LoadingPhaseSpriteRenders[enLoadingPhase_6].Draw(rc);
	}
